...
def sample_tables(operation_rows, date="22/Aug/2026", well="IB-666C"):
    return [
...
"""
        ...
        self.assertEqual([row["code"] for row in activities], ["L300", "P200"])
        self.assertEqual(activities[0]["time_to"], "06:00")
        self.assertEqual(activities[1]["time_to"], "09:30")
        self.assertEqual(activities[1]["metres_from"], 1.0)
        self.assertEqual(activities[1]["metres_to"], 249.0)
        self.assertIsNone(activities[1]["total_metres"])
        self.assertIn("Sonde DUMM", activities[1]["notes"])
...
"""
        ...
        ], date="05/Aug/2026", well="IRONBARK1")

        header, activities, _ = parse_weatherford_gdb(text, tables, "GDB induction.pdf")

        self.assertEqual(header["hole_num"], "IRONBARK1")
        self.assertEqual([row["code"] for row in activities], ["L300", "N100"])
        self.assertIn("INDUCTION", activities[0]["notes"])
        self.assertFalse(any(row["code"] == "P200" for row in activities))
