...
def assign_depco_ironbark_to_gas_riser():
    """Keep every Ironbark DEPCO record in the Gas Riser program scope."""
    moved = {}
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
            ...
            cur.execute("""
                ...
                SELECT DISTINCT ON (contractor, project, section, vendor)
                       contractor, 'Gas Riser', project, section, vendor,
                       budget_amount, allocation, notes, NOW()
                ...
                DO UPDATE SET
                    ...
                    updated_at=NOW()
            """)
            moved["budget_allocations_saved"] = cur.rowcount
            cur.execute("""
